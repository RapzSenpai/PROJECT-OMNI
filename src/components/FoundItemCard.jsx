...
const FoundItemCard = ({ item, refreshItems, user }) => {
    ...
    const canEdit = isOwner || isAdmin || isStaff;
    ...
    const handleDelete = async () => {
        const confirmDelete = window.confirm("Are you sure you want to delete this found item record?");
        if (!confirmDelete) return;

        try {
            await deleteLostItem(item.id);
            alert("Item record deleted!");
            refreshItems();
        } catch (err) {
        ...
        <>
            ...
            <div className={`feed-card lf-card`}>
                {item.status === "unclaimed" && (
                    <div className="lf-warning-box text-center !py-2 !mb-3 !text-[11px] !border-none !bg-gray-50 !text-gray-600">
                        <strong className="!text-gray-700">⚠️ REMINDER:</strong>
                        If this is your item, kindly claim it at the Lost & Found Office (2nd Floor CCS ComLab). Only staff can hand it over.
                    </div>
                )}
                ...
                <div className="lf-card-row row-3">
                    <div className="lf-meta-block">
                        <span className="lf-meta-item">
                            Posted by <span className="font-semibold text-gray-700">{getDisplayName(item?.ownerName || "Staff")}</span>
                        </span>
